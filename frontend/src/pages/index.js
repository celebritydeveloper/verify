...
export default function Home() {
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      ...
      socket.emit("add_user", {email: "admin@gmail.com", id: socket.id}, (res) => {
        console.log(res);
      });
    });
  }, [])
    ...
    <main className={` min-h-screen py-7 ${inter.className}`}>
      ...
      <div className="relative flex justify-center mb-12">
        <form onSubmit={validateEmail}>
          <div className="mb-1">
            <label htmlFor="email" className="mb-2 flex">Enter Email(s) here</label>
            <textarea id="email" name="email" onChange={(e) => setEmail(e.target.value)} className="w-full border-2 p-3 rounded-md outline-none border-blue-500" cols={70} rows={10} />
          </div>
          ...
          <div className="flex justify-between items-center">
            <div className="flex gap-3">
              <button type="submit" className="bg-blue-600 px-12 py-2 text-white rounded-md text-[13px]">{buttonText}</button>
              { successful.length > 0 && <button type="button" onClick={copyBtn} className="bg-orange-600 px-12 py-2 text-white rounded-md text-[13px]">Copy</button>}
            </div>
            { response && <button type="button" onClick={reset} className="bg-red-600 px-12 py-2 text-white rounded-md text-[13px]">Reset</button>}
          </div>
        </form>
      </div>
      ...
      <section className="w-full h-full flex gap-5 px-5">
        <div className="w-full h-full">
          ...
          { successful.map((item, index) => (
            <div className="flex justify-between text-[13px] py-1 bg-green-200 px-2 mb-3" key={index}>
        ...
        <div className="w-full h-full">
          ...
          { bad.map((item, index) => (
            <div className="flex justify-between text-[13px] py-1 bg-red-200 px-2 mb-3" key={index}>
              <span>{item.email}</span>
              <div className="flex gap-3">
